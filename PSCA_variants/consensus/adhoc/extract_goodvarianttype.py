...
# first_q_snv = [sorted([-1 * output_snv[f"log_P_SNV({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_snv["POS"]]
# first_q_ins = [sorted([-1 * output_ins[f"log_P_INS({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_ins["POS"]]
# first_q_del = [sorted([-1 * output_del[f"log_P_DEL({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_del["POS"]]

first_q_dict = dict()
for j in output_snv["POS"]:
    first_q_dict[(j, "SNV")] = sorted([-1 * output_snv[f"log_P_SNV({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4]
for j in output_ins["POS"]:
    first_q_dict[(j, "INS")] = sorted([-1 * output_ins[f"log_P_INS({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4]
for j in output_del["POS"]:
    first_q_dict[(j, "DEL")] = sorted([-1 * output_del[f"log_P_DEL({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4]
