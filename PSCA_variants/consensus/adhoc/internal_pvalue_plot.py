...
ax1.scatter(output_snv["POS"], [sorted([output_snv[f"log_P_SNV({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_snv["POS"]])
ax1.axhline(y=log(alpha) - log(len(Bcell_IDlist)) - log(len(output_snv["POS"])), color='r', linestyle='-', label="log(0.05 / len(Bcell_list) / len(snv_variant))")
ax1.legend(loc = "upper left")
...
ax2.scatter(output_ins["POS"], [sorted([output_ins[f"log_P_INS({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_ins["POS"]])
ax2.axhline(y=log(alpha) - log(len(Bcell_IDlist)) - log(len(output_ins["POS"])), color='r', linestyle='-', label="log(0.05 / len(Bcell_list) / len(ins_variant))")
ax2.legend(loc = "upper left")
...
ax3.scatter(output_del["POS"], [sorted([output_del[f"log_P_DEL({Bcell})"][j] for Bcell in Bcell_IDlist])[len(Bcell_IDlist) // 4] for j in output_del["POS"]])
ax3.axhline(y=log(alpha) - log(len(Bcell_IDlist)) - log(len(output_del["POS"])), color='r', linestyle='-', label="log(0.05 / len(Bcell_list) / len(del_variant))")
ax3.legend(loc = "upper left")
